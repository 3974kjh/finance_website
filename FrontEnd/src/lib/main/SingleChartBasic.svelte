<script lang="ts">
  ...
  import { NaverFinanceImg, CompanyGuideImg } from '$lib/images/logo';
	import type { OverallStockFinalObjectType, SplitLayoutSizeType } from '$lib/types';
  ...
  let calcSignalScoreResult = {
    ...
    isOverGoldenCross: false,
    isNearGoldenCross: false,
    bandPosition: null as number | null
  }
  ...
  onMount(async () => {
    ...
    // 행 레이아웃 사이즈 초기화
    const localStorageSplitRowLayoutSize = localStorage.getItem('singleChartSplitRowLayoutSize');
    splitRowLayoutSize = !!localStorageSplitRowLayoutSize ? JSON.parse(localStorageSplitRowLayoutSize) : initialRowSplitLayoutSize();

    // 컬럼 레이아웃 사이즈 초기화
    const localStorageSplitColLayoutSize = localStorage.getItem('singleChartSplitColLayoutSize');
  ...
  */
  const setSingleChartDataList = async (duration: {month: number, week: number}) => {
    ...
    // 각 이동평균 계산
    const ma5 = calculateMA(financeDataResult, 5, 'Close');
    const ma20 = calculateMA(financeDataResult, 20, 'Close');
    const ma60 = calculateMA(financeDataResult, 60, 'Close');

    // 각 VWMA 계산
    const vwma = calculateVWMA(financeDataResult, 1, 'Close', 'Volume');
    const vwma5 = calculateVWMA(financeDataResult, 5, 'Close', 'Volume');
    const vwma20 = calculateVWMA(financeDataResult, 20, 'Close', 'Volume');
    const vwma60 = calculateVWMA(financeDataResult, 60, 'Close', 'Volume');

    // VWMA 평균값 계산
    const vwmaAvg = getAverageValue(vwma);
    ...
    overallStocFinalObject.generalizedPricePosition = calcSignalScoreResult.bandPosition;
    ...
    return financeDataResult.map((data: any, index: number) => {
      return {
        ...
        nowValue: expectResult.data?.nowValue,
        topValue: expectResult.data?.topValue,
        volume: parseFloat(vwma[index]?.toString() ?? '0') ?? 0,
  ...
  */
  ...
  const onLayoutRowResized = (event: CustomEvent) => {
    ...
    // 행 레이아웃 사이즈 저장
    localStorage.setItem('singleChartSplitRowLayoutSize', JSON.stringify(splitRowLayoutSize));
  };
  ...
  const onLayoutColResized = (event: CustomEvent) => {
    ...
    // 컬럼 레이아웃 사이즈 저장
    localStorage.setItem('singleChartSplitColLayoutSize', JSON.stringify(splitColLayoutSize));
  };
</script>
    ...
    <div class="flex flex-row w-full grow space-x-3" style="height: {clientHeight - 150}px">
      <Splitpanes on:resized={onLayoutRowResized}>
        ...
        <Pane size={splitRowLayoutSize[0].size} minSize={splitRowLayoutSize[0].min} maxSize={splitRowLayoutSize[0].max}>
          <div class="flex flex-col h-full bg-white/80 backdrop-blur-sm border border-gray-200/50 rounded-xl shadow-lg overflow-hidden">
